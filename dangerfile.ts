...
if (packageChanged) {
  message(
    `:man_technologist: Some dependencies were added or removed in package.json file.`,
  );
}
...
const modifiedHtmlFiles = danger.git.modified_files.filter((path) =>
  path.endsWith("html"),
);
...
modifiedHtmlFiles.forEach((file) => {
  const notKebabCaseRegex =
    /class="(?!\s*[a-z0-9]+(?:[\s-][a-z0-9]+)*\s*")[^"]+"/g;
  const content = fs.readFileSync(file).toString();
  let match;
...
modifiedHtmlFiles.forEach((file) => {
  ...
  while ((match = regExp.exec(content)) != null) {
    fail(
      `\`${match[1]}\` - style attribute was found in file (\`${file}\`). Please use (s)css files for styling.`,
    );
  }
});
// check whether test files were added or not
const hasTests = danger.git.modified_files.filter(
  (path) => path.endsWith(".spec.ts") || path.endsWith(".e2e-spec.ts"),
);
...
const patchVersionChanged = commits.find(
  (commit) =>
    !!commit.message.includes("fix:") || !!commit.message.includes("fix("),
);

// check whether commits include message with 'feat:' and 'feat(' prefix
const minorVersionChanged = commits.find(
  (commit) =>
    !!commit.message.includes("feat:") || !!commit.message.includes("feat("),
);

// check whether commits include message with 'BREAKING CHANGES' prefix
const majorVersionChanged = commits.find(
  (commit) => !!commit.message.includes("BREAKING CHANGES"),
);
